---
title: "Pulsations urbaines circadiennes : le cas de Bogotá (Colombie)"
...
## Chargement de la géométrie
    ```{r}
    EMU2019 <- st_read(dsn = getwd(), layer = "EMU2019", stringsAsFactors = FALSE)
    ```


## Chargement des fichiers de l'enquête
    ```{r}
    viajes <- read.xlsx("ViajesEODH2019.xlsx") #134497 observations
    ```

## Définition d'une fonction pour rendre les couleurs transparentes
...
Calcul des durées de voyage (prise en compte des voyages qui commencent un jour et terminent le suivant)
...
Calcul du temps de voyage moyen en heures. Le temps de voyage moyen (aller-simple) est de 50 minutes

    ```{r}
    avg = 24*mean(viajes$duracion) 
    ```

Tri pour écarter les voyages aberrants (0 minute)
    
    ```{r}
    viajes2 <- viajes %>% filter(duracion>0)
    ```

Traitement des trajets dont le point de départ ou d'arrivée se trouve en dehors de Bogotá DC. On crée un UTAM virtuel regroupant les zones hors DC.
...
On retire les voyages intra-UTAM qui sont considérés, à cette échelle, comme des voyages n'ayant pas eu lieu car ils ne contribuent pas à la déformation. 
...
Pour choisir l'heure de référence (où chacun est chez soi), on trace un histogramme de la répartition des heures de déplacements. On se rend compte que le creux de déplacement est maximal à 2h45. On va affecter la variable *NUMPERSTOT*, qui donne la population de nuit issue du recensement de 2018, à ce créneau.
...
![](https://i.imgur.com/qfatK37.png)
    ...
    #un peu de slicing pour réordonner les colonnes en commençant à 3h00.
    stock <- stock[,1:98] 
    stock1 <- stock[,1]
    stock2 <- stock[,2:13]
    stock3 <- stock[,14:98]
    ...
    #On remplace les éventuelles valeurs négatives par des zéros. #Aucun UTAM concerné a priori
    for (i in 3:ncol(stock)){
...
On transpose la table pour tracer des graphes de la population par UTAM au fil de la journée.
    ...
    #on trace le graphe pour deux UTAM : EL LUCERO et CHAPINERO
...
## Premier cartogramme
    ...
    ```{r}
    #Jointure avec le fichier de géométrie
    ...
    #on génère des étiquettes à afficher sur la carte à partir de cette liste
...
Tests d'export de cartogrammes sur des heures individuelles, dans le cas présent à 3h00.On fixe une erreur absolue d'une valeur de 10 hectares, soit 100 000 m². On limite le calcul à 20 itérations.
...
Affichage de la carte chloroplèthe à 12h00.
...
![](https://i.imgur.com/HQMsfAw.png)
    ...
    UTAM <- UTAM[which(!UTAM$UTAM %in% Iles),]
    EMU2019_3 <- EMU2019_3[which(!EMU2019_3$UTAM %in% Iles),]
    nbUTAM <- poly2nb(pl = UTAM,
                     row.names = UTAM$UTAM,
                     snap = 50,
                     queen = TRUE)
...
### Variogramme
    ...
    ```{r}
    #pour calculer les centroides des UTAM sans les îles (indispensable pour le calcul du semi-variogramme)
...
### Carte lissée à 12h00
    ...
    #pour calculer la surface lissée (rayon lissage : 2 km et résolution spatiale de l'image : 1 ha) --> calcul long
...
Le choix du rayon de lissage est important. Après plusieurs essais, on retient un rayon de 1000m qui est un bon compromis entre la prise en compte de suffisamment de voisins pour un UTAM donné, et la représentations fidèle des comportements des différents UTAM. Ci-dessous, la comparaison entre un rayon de lissage de 1000m et un rayon de 1500m.

![](https://i.imgur.com/Yzd11Fq.png)
![](https://i.imgur.com/slT8Q3z.png)


## Cartogramme et surface lissée au pas de temps d'un quart d'heure
    ...
    #choix des UTAM représentatifs des localidades dont le nom sera affiché. Il est possible d'en ajouter ou d'en enlever en jouant sur les #
    liste <- c(#"UTAM38",
               "UTAM22",
               "UTAM84",
               "UTAM90",
    ...
    for (i in 1:96){
      ...
      #sélection des zones d'enquête modural
      carto_sf_modural <- carto_sf %>%
        filter(UTAM %in% c("UTAM28", "UTAM67", "UTAM86", "UTAM575"))
      
      #Affichage des limites des UTAM déformés
      ...
      #chaque localidad est représentée par un UTAM choisi à la main
      localidades <- carto_sf %>%
        filter(UTAM %in% liste)

      #Affichage du nom des Localidades
      labelLayer(localidades, txt = "LocMuni", halo = TRUE, bg = "white", r = 0.05, cex = 1.2, pos = 3, font = 4, offset = 0)
...
Ci-dessous, quelques uns des 96 visuels obtenus.

![](https://i.imgur.com/IzWVuH5.png)
![](https://i.imgur.com/btfErOq.png)
![](https://i.imgur.com/KKwGJDt.png)
![](https://i.imgur.com/KaneEYS.png)


## Déformation initiale de la carte
Pour ce faire, on réalise une interpolation entre la carte de Bogotá non déformée et la carte déformée selon la population de nuit à 3h00.
    ...
    ```{r}
    EMU2019_area <- EMU2019[which(!EMU2019$UTAM %in% Iles),]
    ...
    #calcul de la superficie totale (champ rajouté sur QGIS)
...
![](https://i.imgur.com/4YW1ceX.png)
![](https://i.imgur.com/6bFMMz5.png)
![](https://i.imgur.com/BVDVN5S.png)


## Fondu enchaîné pour l'apparition de la seconde variable

    ```{r}
    cols <- cols #on reprend les couleurs de l'animation
    ...
    for (i in c(50,60,70,80,90)) {
      ...
      plot(carto_sf_modural$geometry, border = paste("grey", 1.8*i-90), lwd = 3, add = TRUE)
      plot(carto_sf_modural$geometry, border = paste("grey", 200-2*i), lwd = 1, add = TRUE)
  
      #chaque localidad est représentée par un UTAM, seule façon d'éviter les problèmes de géométrie dûs aux déformations
      localidades <- carto_sf %>%
        filter(UTAM %in% liste)

      #Affichage du nom des Localidades
      labelLayer(localidades, txt = "LocMuni", halo = TRUE, bg = "white", r = 0.05, cex = 1.2, pos = 3, font = 4, offset = 0, col = paste("grey", 1.8*i-90))
...
![](https://i.imgur.com/t1xXvJY.png)
![](https://i.imgur.com/WQJSMOA.png)
![](https://i.imgur.com/ygPQRp5.png)
![](https://i.imgur.com/YIRmih1.png)
![](https://i.imgur.com/kceKtxS.png)



## Création d'un gif animé ou d'une vidéo mp4 à partir de la série d'images produite

    ```{r}
    #chargement des informations des fichiers à intégrer à l'animation

    details = file.info(list.files(getwd(), full.names = TRUE))
    ...
    img_animated <- image_animate(img_joined, fps = 20) #animation

    #export en gif
    image_write(image = img_animated, path = "animation.gif")

    #export en mp4
    av_encode_video(input = list.files(getwd()),framerate = 15, "animation.mp4")
    ```
    
Ci-dessous un export au format gif de l'animation bouclée, hors déformation initiale.
